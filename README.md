...
| 章节 | 标题 | 你会手写出 |
|---|---|---|
| [00-全景-Agent框架与Harness](harness/00-全景-Agent框架与Harness.md) | 全景:Agent、框架与 Harness | 2026 格局地图、四层架构、方法论 |
| [01-最小Agent循环](harness/01-最小Agent循环.md) | 最小 Agent 循环 | 消息、模型接口、`@tool`、~100 行主循环 |
| [02-工具系统与执行环境](harness/02-工具系统与执行环境.md) | 工具系统与执行环境 | 编码工具八件套、路径 jail、并行执行、错误即消息 |
| [03-上下文工程](harness/03-上下文工程.md) | 上下文工程 | system prompt 装配、记忆文件、压缩、结构化笔记 |
| [04-事件流与流式](harness/04-事件流与流式.md) | 事件流与流式 | 事件协议、逐 token 流式、打断与中途插话 |

**主线:Harness 篇(造壳)**

| 章节 | 标题 | 你会手写出 |
|---|---|---|
| [05-会话与持久化](harness/05-会话与持久化.md) | 会话与持久化 | JSONL 事件日志、重放、恢复、分叉 |
| [06-权限与沙箱](harness/06-权限与沙箱.md) | 权限与沙箱 | 权限模式、规则、审批回调、两种信任姿态 |
| [07-钩子与技能](harness/07-钩子与技能.md) | 钩子与技能 | 生命周期钩子、SKILL.md 渐进披露 |
| [08-子代理与多智能体](harness/08-子代理与多智能体.md) | 子代理与多智能体 | task 工具、上下文隔离、并行扇出 |
| [09-MCP-工具的生态协议](harness/09-MCP-工具的生态协议.md) | MCP:工具的生态协议 | 最小 MCP 客户端 + server、Harness 门面组装 |
| [10-第二条路线-图运行时](harness/10-第二条路线-图运行时.md) | 第二条路线:图运行时 | 选型判据;通往深入篇的桥 |
| [11-收尾-对照真实框架](harness/11-收尾-对照真实框架.md) | 收尾:对照真实框架 | 与 Claude Agent SDK / Agents SDK / LangGraph 逐层对照、源码阅读路线 |

**深入篇:图运行时(runtime/,可在主线后读,也可从第 10 章跳入)**

[导读 R0](graph/R0-深入篇导读.md) · R1 循环的局限 · R2 构建图运行时(Pregel/BSP) · R3 持久化与检查点 · R4 流式与人在回路 · R5 重建 Agent 抽象层 · R6 中间件系统 · R7 进阶与生产化
